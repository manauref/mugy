...
mugy_fftw_plan fft_plan_xyfft_c2r, fft_plan_xyfft_r2c;
...
void init_ffts(struct grid gridG, struct grid gridL) {
  ...
  blockSize0out = gridL.fG.dual.Nx[1];
  fft_plan_xyfft_c2r = mugy_fftw_mpi_plan_many_dft_c2r(fftDim, fftSize, fftNum, blockSize0in, blockSize0out,
                                                   fk_xyfft_buf, f_xyfft_buf, *xyComm, FFTW_ESTIMATE);
  blockSize0in  = gridL.fG.dual.Nx[1];
  blockSize0out = blockSizek0;
  fft_plan_xyfft_r2c = mugy_fftw_mpi_plan_many_dft_r2c(fftDim, fftSize, fftNum, blockSize0in, blockSize0out,
...
#if USE_GPU
//  if (res == defaultComp || res == deviceComp)
//    return xyfft_c2r_dev(fOut, fkIn);
  ...
  // Inverse FFT.
  mugy_fftw_mpi_execute_dft_c2r(fft_plan_xyfft_c2r, fk_xyfft_buf, f_xyfft_buf);
...
//    return xyfft_c2r_dev(fOut, fkIn);
  ...
  // Inverse FFT.
  mugy_fftw_mpi_execute_dft_r2c(fft_plan_xyfft_r2c, f_xyfft_buf, fk_xyfft_buf);
...
void terminate_ffts() {
  ...
  mugy_fftw_destroy_plan(fft_plan_xyfft_c2r);
  mugy_fftw_destroy_plan(fft_plan_xyfft_r2c);
