...
MUGY_CU_D void* getMomentk(void *momkIn, mint sIdx, mint momIdx, mint xIdx, mint NekxTot, mint numMoments) {
  ...
  return (cufourier *)momkIn + (momOff+momIdx)*NekxTot+xIdx;
}

__global__ void mugy_field_poisson_solve_cu(cufourier *phik, const cufourier *momk, const cufourier *poissonFac, mint nelem, mint numSpecies, mint numMoments, mint NekxTot)
{
...
//    out[linc] = mugy_cuCmul(mugy_make_cuComplex(a,0.), inp[linc]);
  ...
  for (mint linIdx=0; linIdx<NekxTot; linIdx++) {
    cufourier *phik_p = &phik[linIdx];
    phik_p[0] = mugy_make_cuComplex(0., 0.);
    for (mint s=0; s<numMoments; s++) {
      for (mint m=0; m<numMoments; m++) {
        const cufourier *poissonFac_p = (const cufourier *)getMomentk((void*)poissonFac, s, m, linIdx, NekxTot, numMoments);
        const cufourier *momk_p       = (const cufourier *)getMomentk((void*)momk      , s, m, linIdx, NekxTot, numMoments);
...
void mugy_field_poisson_solve_dev(struct mugy_field *field, struct mugy_population *pop, struct mugy_grid *grid, mint tstepIdx) {
  ...
  mugy_field_poisson_solve_cu<<<nblocks, nthreads>>>((cufourier *)phik->dev, (const cufourier *)momk->dev, (const cufourier *)popL->poissonFac->dev, phik->nelem, popL->numSpecies, popL->pars[0].numMoments, gridL->NekxTot);
}
